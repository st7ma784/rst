# Guides

Comprehensive reference documentation for RST usage and development.

```{toctree}
:maxdepth: 2

commands
contributing
benchmarks
python-api
docker
slurm-rst-install
```

## Documentation Overview

| Guide | Description |
|-------|-------------|
| [Command Reference](commands.md) | All RST commands and options |
| [Contributing](contributing.md) | How to contribute to RST |
| [Benchmarks](benchmarks.md) | Performance analysis and optimization |
| [Python API](python-api.md) | Python bindings documentation |
| [Docker Guide](docker.md) | Container deployment and usage |
| [RST on Slurm](slurm-rst-install.md) | Cluster-wide installation and runtime setup |

## Quick Links

### For Users
- [Processing Commands](commands.md#processing-commands)
- [Visualization Commands](commands.md#visualization-commands)
- [Data Formats](commands.md#data-formats)

### For Developers
- [Contributing Guidelines](contributing.md)
- [Testing](contributing.md#testing)
- [Code Style](contributing.md#code-style)

### For Operations
- [Docker Deployment](docker.md)
- [RST on Slurm](slurm-rst-install.md)
- [Performance Tuning](benchmarks.md#optimization)
- [FITACF GPU Benchmark and Regression](benchmarks.md#fitacf-batch-benchmark-and-regression)
