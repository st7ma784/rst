#ifndef OLDGRID.1.3_CUDA_H
#define OLDGRID.1.3_CUDA_H
...
/* Core oldgrid.1.3 CUDA Functions */
cudaError_t oldgrid.1.3_process_cuda(
    cuda_array_t *input_data,
    cuda_array_t *output_data,
    void *parameters
);

cudaError_t oldgrid.1.3_process_ranges_cuda(
...
/* Utility Functions */
bool oldgrid.1.3_cuda_is_available(void);
int oldgrid.1.3_cuda_get_device_count(void);
const char* oldgrid.1.3_cuda_get_error_string(cudaError_t error);
...
} oldgrid.1.3_cuda_profile_t;

cudaError_t oldgrid.1.3_cuda_enable_profiling(bool enable);
cudaError_t oldgrid.1.3_cuda_get_profile(oldgrid.1.3_cuda_profile_t *profile);

#ifdef __cplusplus
}
#endif

#endif /* OLDGRID.1.3_CUDA_H */
