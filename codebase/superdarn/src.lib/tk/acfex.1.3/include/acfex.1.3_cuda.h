#ifndef ACFEX.1.3_CUDA_H
#define ACFEX.1.3_CUDA_H
...
/* Core acfex.1.3 CUDA Functions */
cudaError_t acfex.1.3_process_cuda(
    cuda_array_t *input_data,
    cuda_array_t *output_data,
    void *parameters
);

cudaError_t acfex.1.3_process_ranges_cuda(
...
/* Utility Functions */
bool acfex.1.3_cuda_is_available(void);
int acfex.1.3_cuda_get_device_count(void);
const char* acfex.1.3_cuda_get_error_string(cudaError_t error);
...
} acfex.1.3_cuda_profile_t;

cudaError_t acfex.1.3_cuda_enable_profiling(bool enable);
cudaError_t acfex.1.3_cuda_get_profile(acfex.1.3_cuda_profile_t *profile);

#ifdef __cplusplus
}
#endif

#endif /* ACFEX.1.3_CUDA_H */
