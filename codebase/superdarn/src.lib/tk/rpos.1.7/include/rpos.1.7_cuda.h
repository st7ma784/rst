#ifndef RPOS.1.7_CUDA_H
#define RPOS.1.7_CUDA_H
...
/* Core rpos.1.7 CUDA Functions */
cudaError_t rpos.1.7_process_cuda(
    cuda_array_t *input_data,
    cuda_array_t *output_data,
    void *parameters
);

cudaError_t rpos.1.7_process_ranges_cuda(
...
/* Utility Functions */
bool rpos.1.7_cuda_is_available(void);
int rpos.1.7_cuda_get_device_count(void);
const char* rpos.1.7_cuda_get_error_string(cudaError_t error);
...
} rpos.1.7_cuda_profile_t;

cudaError_t rpos.1.7_cuda_enable_profiling(bool enable);
cudaError_t rpos.1.7_cuda_get_profile(rpos.1.7_cuda_profile_t *profile);

#ifdef __cplusplus
}
#endif

#endif /* RPOS.1.7_CUDA_H */
