#ifndef FITCNX.1.16_CUDA_H
#define FITCNX.1.16_CUDA_H
...
/* Core fitcnx.1.16 CUDA Functions */
cudaError_t fitcnx.1.16_process_cuda(
    cuda_array_t *input_data,
    cuda_array_t *output_data,
    void *parameters
);

cudaError_t fitcnx.1.16_process_ranges_cuda(
...
/* Utility Functions */
bool fitcnx.1.16_cuda_is_available(void);
int fitcnx.1.16_cuda_get_device_count(void);
const char* fitcnx.1.16_cuda_get_error_string(cudaError_t error);
...
} fitcnx.1.16_cuda_profile_t;

cudaError_t fitcnx.1.16_cuda_enable_profiling(bool enable);
cudaError_t fitcnx.1.16_cuda_get_profile(fitcnx.1.16_cuda_profile_t *profile);

#ifdef __cplusplus
}
#endif

#endif /* FITCNX.1.16_CUDA_H */
