...
/* CUDA Initialization */
__host__ cudaError_t fitcnx.1.16_cuda_init(void) {
...
/* CUDA Kernels */
template<typename T>
__global__ void fitcnx.1.16_process_kernel(
    T *input_data, 
    T *output_data, 
    size_t num_elements
) {
...
cudaError_t fitcnx.1.16_process_cuda(
    ...
    if (!cuda_initialized) {
        cudaError_t init_error = fitcnx.1.16_cuda_init();
    ...
    if (input_data->type == CUDA_R_32F) {
        fitcnx.1.16_process_kernel<float><<<blocks, threads_per_block>>>(
