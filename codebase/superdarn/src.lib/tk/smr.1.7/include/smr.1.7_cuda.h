#ifndef SMR.1.7_CUDA_H
#define SMR.1.7_CUDA_H
...
/* Core smr.1.7 CUDA Functions */
cudaError_t smr.1.7_process_cuda(
    cuda_array_t *input_data,
    cuda_array_t *output_data,
    void *parameters
);

cudaError_t smr.1.7_process_ranges_cuda(
...
/* Utility Functions */
bool smr.1.7_cuda_is_available(void);
int smr.1.7_cuda_get_device_count(void);
const char* smr.1.7_cuda_get_error_string(cudaError_t error);
...
} smr.1.7_cuda_profile_t;

cudaError_t smr.1.7_cuda_enable_profiling(bool enable);
cudaError_t smr.1.7_cuda_get_profile(smr.1.7_cuda_profile_t *profile);

#ifdef __cplusplus
}
#endif

#endif /* SMR.1.7_CUDA_H */
