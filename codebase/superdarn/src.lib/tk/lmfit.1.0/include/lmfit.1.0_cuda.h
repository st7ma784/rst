#ifndef LMFIT.1.0_CUDA_H
#define LMFIT.1.0_CUDA_H
...
/* Core lmfit.1.0 CUDA Functions */
cudaError_t lmfit.1.0_process_cuda(
    cuda_array_t *input_data,
    cuda_array_t *output_data,
    void *parameters
);

cudaError_t lmfit.1.0_process_ranges_cuda(
...
/* Utility Functions */
bool lmfit.1.0_cuda_is_available(void);
int lmfit.1.0_cuda_get_device_count(void);
const char* lmfit.1.0_cuda_get_error_string(cudaError_t error);
...
} lmfit.1.0_cuda_profile_t;

cudaError_t lmfit.1.0_cuda_enable_profiling(bool enable);
cudaError_t lmfit.1.0_cuda_get_profile(lmfit.1.0_cuda_profile_t *profile);

#ifdef __cplusplus
}
#endif

#endif /* LMFIT.1.0_CUDA_H */
