#ifndef ACF.1.16_CUDA_H
#define ACF.1.16_CUDA_H
...
/* Core acf.1.16 CUDA Functions */
cudaError_t acf.1.16_process_cuda(
    cuda_array_t *input_data,
    cuda_array_t *output_data,
    void *parameters
);

cudaError_t acf.1.16_process_ranges_cuda(
...
/* Utility Functions */
bool acf.1.16_cuda_is_available(void);
int acf.1.16_cuda_get_device_count(void);
const char* acf.1.16_cuda_get_error_string(cudaError_t error);
...
} acf.1.16_cuda_profile_t;

cudaError_t acf.1.16_cuda_enable_profiling(bool enable);
cudaError_t acf.1.16_cuda_get_profile(acf.1.16_cuda_profile_t *profile);

#ifdef __cplusplus
}
#endif

#endif /* ACF.1.16_CUDA_H */
