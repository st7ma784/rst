...
/* CUDA Kernels */
template<typename T>
__global__ void fit.1.35_process_kernel(
    T *input_data, 
    T *output_data, 
    size_t num_elements
) {
...
/* Host Functions */
cudaError_t fit.1.35_process_cuda(
    cuda_array_t *input_data,
    cuda_array_t *output_data,
    void *parameters
) {
    if (!cuda_initialized) {
        cudaError_t init_error = fit.1.35_cuda_init();
    ...
    if (input_data->type == CUDA_R_32F) {
        fit.1.35_process_kernel<float><<<blocks, threads_per_block>>>(
