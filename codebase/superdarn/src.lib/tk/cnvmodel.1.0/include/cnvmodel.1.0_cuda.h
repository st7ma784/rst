#ifndef CNVMODEL.1.0_CUDA_H
#define CNVMODEL.1.0_CUDA_H
...
/* Core cnvmodel.1.0 CUDA Functions */
cudaError_t cnvmodel.1.0_process_cuda(
    cuda_array_t *input_data,
    cuda_array_t *output_data,
    void *parameters
);

cudaError_t cnvmodel.1.0_process_ranges_cuda(
...
/* Utility Functions */
bool cnvmodel.1.0_cuda_is_available(void);
int cnvmodel.1.0_cuda_get_device_count(void);
const char* cnvmodel.1.0_cuda_get_error_string(cudaError_t error);
...
} cnvmodel.1.0_cuda_profile_t;

cudaError_t cnvmodel.1.0_cuda_enable_profiling(bool enable);
cudaError_t cnvmodel.1.0_cuda_get_profile(cnvmodel.1.0_cuda_profile_t *profile);

#ifdef __cplusplus
}
#endif

#endif /* CNVMODEL.1.0_CUDA_H */
