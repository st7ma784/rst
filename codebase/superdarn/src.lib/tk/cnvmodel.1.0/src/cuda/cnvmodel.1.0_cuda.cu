...
/* CUDA Initialization */
__host__ cudaError_t cnvmodel.1.0_cuda_init(void) {
...
/* CUDA Kernels */
template<typename T>
__global__ void cnvmodel.1.0_process_kernel(
    T *input_data, 
    T *output_data, 
    size_t num_elements
) {
...
cudaError_t cnvmodel.1.0_process_cuda(
    ...
    if (!cuda_initialized) {
        cudaError_t init_error = cnvmodel.1.0_cuda_init();
    ...
    if (input_data->type == CUDA_R_32F) {
        cnvmodel.1.0_process_kernel<float><<<blocks, threads_per_block>>>(
