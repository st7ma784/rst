#include "gtablewrite.1.9_cuda.h"
#include <stdio.h>
#include <stdlib.h>
...
/* CUDA Initialization */
__host__ cudaError_t gtablewrite.1.9_cuda_init(void) {
...
/* CUDA Kernels */
template<typename T>
__global__ void gtablewrite.1.9_process_kernel(
    T *input_data, 
    T *output_data, 
    size_t num_elements
) {
...
cudaError_t gtablewrite.1.9_process_cuda(
    ...
    if (!cuda_initialized) {
        cudaError_t init_error = gtablewrite.1.9_cuda_init();
    ...
    if (input_data->type == CUDA_R_32F) {
        gtablewrite.1.9_process_kernel<float><<<blocks, threads_per_block>>>(
