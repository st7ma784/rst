#ifndef GTABLEWRITE.1.9_CUDA_H
#define GTABLEWRITE.1.9_CUDA_H
...
/* Core gtablewrite.1.9 CUDA Functions */
cudaError_t gtablewrite.1.9_process_cuda(
    cuda_array_t *input_data,
    cuda_array_t *output_data,
    void *parameters
);

cudaError_t gtablewrite.1.9_process_ranges_cuda(
...
/* Utility Functions */
bool gtablewrite.1.9_cuda_is_available(void);
int gtablewrite.1.9_cuda_get_device_count(void);
const char* gtablewrite.1.9_cuda_get_error_string(cudaError_t error);
...
} gtablewrite.1.9_cuda_profile_t;

cudaError_t gtablewrite.1.9_cuda_enable_profiling(bool enable);
cudaError_t gtablewrite.1.9_cuda_get_profile(gtablewrite.1.9_cuda_profile_t *profile);

#ifdef __cplusplus
}
#endif

#endif /* GTABLEWRITE.1.9_CUDA_H */
