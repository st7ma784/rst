#ifndef BINPLOTLIB.1.0_CUDA_H
#define BINPLOTLIB.1.0_CUDA_H
...
/* Core binplotlib.1.0 CUDA Functions */
cudaError_t binplotlib.1.0_process_cuda(
    cuda_array_t *input_data,
    cuda_array_t *output_data,
    void *parameters
);

cudaError_t binplotlib.1.0_process_ranges_cuda(
...
/* Utility Functions */
bool binplotlib.1.0_cuda_is_available(void);
int binplotlib.1.0_cuda_get_device_count(void);
const char* binplotlib.1.0_cuda_get_error_string(cudaError_t error);
...
} binplotlib.1.0_cuda_profile_t;

cudaError_t binplotlib.1.0_cuda_enable_profiling(bool enable);
cudaError_t binplotlib.1.0_cuda_get_profile(binplotlib.1.0_cuda_profile_t *profile);

#ifdef __cplusplus
}
#endif

#endif /* BINPLOTLIB.1.0_CUDA_H */
