#ifndef FITACFEX.1.3_CUDA_H
#define FITACFEX.1.3_CUDA_H
...
/* Core fitacfex.1.3 CUDA Functions */
cudaError_t fitacfex.1.3_process_cuda(
    cuda_array_t *input_data,
    cuda_array_t *output_data,
    void *parameters
);

cudaError_t fitacfex.1.3_process_ranges_cuda(
...
/* Utility Functions */
bool fitacfex.1.3_cuda_is_available(void);
int fitacfex.1.3_cuda_get_device_count(void);
const char* fitacfex.1.3_cuda_get_error_string(cudaError_t error);
...
} fitacfex.1.3_cuda_profile_t;

cudaError_t fitacfex.1.3_cuda_enable_profiling(bool enable);
cudaError_t fitacfex.1.3_cuda_get_profile(fitacfex.1.3_cuda_profile_t *profile);

#ifdef __cplusplus
}
#endif

#endif /* FITACFEX.1.3_CUDA_H */
