...
/* CUDA Initialization */
__host__ cudaError_t fitacfex.1.3_cuda_init(void) {
...
/* CUDA Kernels */
template<typename T>
__global__ void fitacfex.1.3_process_kernel(
    T *input_data, 
    T *output_data, 
    size_t num_elements
) {
...
cudaError_t fitacfex.1.3_process_cuda(
    ...
    if (!cuda_initialized) {
        cudaError_t init_error = fitacfex.1.3_cuda_init();
    ...
    if (input_data->type == CUDA_R_32F) {
        fitacfex.1.3_process_kernel<float><<<blocks, threads_per_block>>>(
