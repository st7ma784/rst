#ifndef TSG.1.13_CUDA_H
#define TSG.1.13_CUDA_H
...
/* Core tsg.1.13 CUDA Functions */
cudaError_t tsg.1.13_process_cuda(
    cuda_array_t *input_data,
    cuda_array_t *output_data,
    void *parameters
);

cudaError_t tsg.1.13_process_ranges_cuda(
...
/* Utility Functions */
bool tsg.1.13_cuda_is_available(void);
int tsg.1.13_cuda_get_device_count(void);
const char* tsg.1.13_cuda_get_error_string(cudaError_t error);
...
} tsg.1.13_cuda_profile_t;

cudaError_t tsg.1.13_cuda_enable_profiling(bool enable);
cudaError_t tsg.1.13_cuda_get_profile(tsg.1.13_cuda_profile_t *profile);

#ifdef __cplusplus
}
#endif

#endif /* TSG.1.13_CUDA_H */
