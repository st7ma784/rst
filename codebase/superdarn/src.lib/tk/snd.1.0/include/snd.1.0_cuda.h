#ifndef SND.1.0_CUDA_H
#define SND.1.0_CUDA_H
...
/* Core snd.1.0 CUDA Functions */
cudaError_t snd.1.0_process_cuda(
    cuda_array_t *input_data,
    cuda_array_t *output_data,
    void *parameters
);

cudaError_t snd.1.0_process_ranges_cuda(
...
/* Utility Functions */
bool snd.1.0_cuda_is_available(void);
int snd.1.0_cuda_get_device_count(void);
const char* snd.1.0_cuda_get_error_string(cudaError_t error);
...
} snd.1.0_cuda_profile_t;

cudaError_t snd.1.0_cuda_enable_profiling(bool enable);
cudaError_t snd.1.0_cuda_get_profile(snd.1.0_cuda_profile_t *profile);

#ifdef __cplusplus
}
#endif

#endif /* SND.1.0_CUDA_H */
