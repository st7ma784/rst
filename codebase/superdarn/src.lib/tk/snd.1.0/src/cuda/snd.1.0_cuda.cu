...
/* CUDA Initialization */
__host__ cudaError_t snd.1.0_cuda_init(void) {
...
/* CUDA Kernels */
template<typename T>
__global__ void snd.1.0_process_kernel(
    T *input_data, 
    T *output_data, 
    size_t num_elements
) {
...
cudaError_t snd.1.0_process_cuda(
    ...
    if (!cuda_initialized) {
        cudaError_t init_error = snd.1.0_cuda_init();
    ...
    if (input_data->type == CUDA_R_32F) {
        snd.1.0_process_kernel<float><<<blocks, threads_per_block>>>(
