#ifndef FREQBAND.1.0_CUDA_H
#define FREQBAND.1.0_CUDA_H
...
/* Core freqband.1.0 CUDA Functions */
cudaError_t freqband.1.0_process_cuda(
    cuda_array_t *input_data,
    cuda_array_t *output_data,
    void *parameters
);

cudaError_t freqband.1.0_process_ranges_cuda(
...
/* Utility Functions */
bool freqband.1.0_cuda_is_available(void);
int freqband.1.0_cuda_get_device_count(void);
const char* freqband.1.0_cuda_get_error_string(cudaError_t error);
...
} freqband.1.0_cuda_profile_t;

cudaError_t freqband.1.0_cuda_enable_profiling(bool enable);
cudaError_t freqband.1.0_cuda_get_profile(freqband.1.0_cuda_profile_t *profile);

#ifdef __cplusplus
}
#endif

#endif /* FREQBAND.1.0_CUDA_H */
