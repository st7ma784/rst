...
/* CUDA Initialization */
__host__ cudaError_t sim_data.1.0_cuda_init(void) {
...
/* CUDA Kernels */
template<typename T>
__global__ void sim_data.1.0_process_kernel(
    T *input_data, 
    T *output_data, 
    size_t num_elements
) {
...
cudaError_t sim_data.1.0_process_cuda(
    ...
    if (!cuda_initialized) {
        cudaError_t init_error = sim_data.1.0_cuda_init();
    ...
    if (input_data->type == CUDA_R_32F) {
        sim_data.1.0_process_kernel<float><<<blocks, threads_per_block>>>(
