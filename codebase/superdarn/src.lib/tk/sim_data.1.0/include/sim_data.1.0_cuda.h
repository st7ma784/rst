#ifndef SIM_DATA.1.0_CUDA_H
#define SIM_DATA.1.0_CUDA_H
...
/* Core sim_data.1.0 CUDA Functions */
cudaError_t sim_data.1.0_process_cuda(
    cuda_array_t *input_data,
    cuda_array_t *output_data,
    void *parameters
);

cudaError_t sim_data.1.0_process_ranges_cuda(
...
/* Utility Functions */
bool sim_data.1.0_cuda_is_available(void);
int sim_data.1.0_cuda_get_device_count(void);
const char* sim_data.1.0_cuda_get_error_string(cudaError_t error);
...
} sim_data.1.0_cuda_profile_t;

cudaError_t sim_data.1.0_cuda_enable_profiling(bool enable);
cudaError_t sim_data.1.0_cuda_get_profile(sim_data.1.0_cuda_profile_t *profile);

#ifdef __cplusplus
}
#endif

#endif /* SIM_DATA.1.0_CUDA_H */
