#ifndef OLDFITCNX.1.10_CUDA_H
#define OLDFITCNX.1.10_CUDA_H
...
/* Core oldfitcnx.1.10 CUDA Functions */
cudaError_t oldfitcnx.1.10_process_cuda(
    cuda_array_t *input_data,
    cuda_array_t *output_data,
    void *parameters
);

cudaError_t oldfitcnx.1.10_process_ranges_cuda(
...
/* Utility Functions */
bool oldfitcnx.1.10_cuda_is_available(void);
int oldfitcnx.1.10_cuda_get_device_count(void);
const char* oldfitcnx.1.10_cuda_get_error_string(cudaError_t error);
...
} oldfitcnx.1.10_cuda_profile_t;

cudaError_t oldfitcnx.1.10_cuda_enable_profiling(bool enable);
cudaError_t oldfitcnx.1.10_cuda_get_profile(oldfitcnx.1.10_cuda_profile_t *profile);

#ifdef __cplusplus
}
#endif

#endif /* OLDFITCNX.1.10_CUDA_H */
