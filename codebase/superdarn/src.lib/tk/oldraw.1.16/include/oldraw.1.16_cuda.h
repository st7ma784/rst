#ifndef OLDRAW.1.16_CUDA_H
#define OLDRAW.1.16_CUDA_H
...
/* Core oldraw.1.16 CUDA Functions */
cudaError_t oldraw.1.16_process_cuda(
    cuda_array_t *input_data,
    cuda_array_t *output_data,
    void *parameters
);

cudaError_t oldraw.1.16_process_ranges_cuda(
...
/* Utility Functions */
bool oldraw.1.16_cuda_is_available(void);
int oldraw.1.16_cuda_get_device_count(void);
const char* oldraw.1.16_cuda_get_error_string(cudaError_t error);
...
} oldraw.1.16_cuda_profile_t;

cudaError_t oldraw.1.16_cuda_enable_profiling(bool enable);
cudaError_t oldraw.1.16_cuda_get_profile(oldraw.1.16_cuda_profile_t *profile);

#ifdef __cplusplus
}
#endif

#endif /* OLDRAW.1.16_CUDA_H */
