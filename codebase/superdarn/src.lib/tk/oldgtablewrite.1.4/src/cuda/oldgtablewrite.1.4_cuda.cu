#include "oldgtablewrite.1.4_cuda.h"
#include <stdio.h>
#include <stdlib.h>
...
/* CUDA Initialization */
__host__ cudaError_t oldgtablewrite.1.4_cuda_init(void) {
...
/* CUDA Kernels */
template<typename T>
__global__ void oldgtablewrite.1.4_process_kernel(
    T *input_data, 
    T *output_data, 
    size_t num_elements
) {
...
cudaError_t oldgtablewrite.1.4_process_cuda(
    ...
    if (!cuda_initialized) {
        cudaError_t init_error = oldgtablewrite.1.4_cuda_init();
    ...
    if (input_data->type == CUDA_R_32F) {
        oldgtablewrite.1.4_process_kernel<float><<<blocks, threads_per_block>>>(
