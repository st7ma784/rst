#ifndef OLDFIT.1.25_CUDA_H
#define OLDFIT.1.25_CUDA_H
...
/* Core oldfit.1.25 CUDA Functions */
cudaError_t oldfit.1.25_process_cuda(
    cuda_array_t *input_data,
    cuda_array_t *output_data,
    void *parameters
);

cudaError_t oldfit.1.25_process_ranges_cuda(
...
/* Utility Functions */
bool oldfit.1.25_cuda_is_available(void);
int oldfit.1.25_cuda_get_device_count(void);
const char* oldfit.1.25_cuda_get_error_string(cudaError_t error);
...
} oldfit.1.25_cuda_profile_t;

cudaError_t oldfit.1.25_cuda_enable_profiling(bool enable);
cudaError_t oldfit.1.25_cuda_get_profile(oldfit.1.25_cuda_profile_t *profile);

#ifdef __cplusplus
}
#endif

#endif /* OLDFIT.1.25_CUDA_H */
