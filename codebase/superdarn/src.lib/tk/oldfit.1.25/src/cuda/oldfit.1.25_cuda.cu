...
/* CUDA Initialization */
__host__ cudaError_t oldfit.1.25_cuda_init(void) {
...
/* CUDA Kernels */
template<typename T>
__global__ void oldfit.1.25_process_kernel(
    T *input_data, 
    T *output_data, 
    size_t num_elements
) {
...
cudaError_t oldfit.1.25_process_cuda(
    ...
    if (!cuda_initialized) {
        cudaError_t init_error = oldfit.1.25_cuda_init();
    ...
    if (input_data->type == CUDA_R_32F) {
        oldfit.1.25_process_kernel<float><<<blocks, threads_per_block>>>(
