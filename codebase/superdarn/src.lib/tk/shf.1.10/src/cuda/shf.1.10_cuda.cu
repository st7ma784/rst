...
/* CUDA Initialization */
__host__ cudaError_t shf.1.10_cuda_init(void) {
...
/* CUDA Kernels */
template<typename T>
__global__ void shf.1.10_process_kernel(
    T *input_data, 
    T *output_data, 
    size_t num_elements
) {
...
cudaError_t shf.1.10_process_cuda(
    ...
    if (!cuda_initialized) {
        cudaError_t init_error = shf.1.10_cuda_init();
    ...
    if (input_data->type == CUDA_R_32F) {
        shf.1.10_process_kernel<float><<<blocks, threads_per_block>>>(
