#ifndef SHF.1.10_CUDA_H
#define SHF.1.10_CUDA_H
...
/* Core shf.1.10 CUDA Functions */
cudaError_t shf.1.10_process_cuda(
    cuda_array_t *input_data,
    cuda_array_t *output_data,
    void *parameters
);

cudaError_t shf.1.10_process_ranges_cuda(
...
/* Utility Functions */
bool shf.1.10_cuda_is_available(void);
int shf.1.10_cuda_get_device_count(void);
const char* shf.1.10_cuda_get_error_string(cudaError_t error);
...
} shf.1.10_cuda_profile_t;

cudaError_t shf.1.10_cuda_enable_profiling(bool enable);
cudaError_t shf.1.10_cuda_get_profile(shf.1.10_cuda_profile_t *profile);

#ifdef __cplusplus
}
#endif

#endif /* SHF.1.10_CUDA_H */
