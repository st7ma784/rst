#ifndef CNVMAP.1.17_CUDA_H
#define CNVMAP.1.17_CUDA_H
...
/* Core cnvmap.1.17 CUDA Functions */
cudaError_t cnvmap.1.17_process_cuda(
    cuda_array_t *input_data,
    cuda_array_t *output_data,
    void *parameters
);

cudaError_t cnvmap.1.17_process_ranges_cuda(
...
/* Utility Functions */
bool cnvmap.1.17_cuda_is_available(void);
int cnvmap.1.17_cuda_get_device_count(void);
const char* cnvmap.1.17_cuda_get_error_string(cudaError_t error);
...
} cnvmap.1.17_cuda_profile_t;

cudaError_t cnvmap.1.17_cuda_enable_profiling(bool enable);
cudaError_t cnvmap.1.17_cuda_get_profile(cnvmap.1.17_cuda_profile_t *profile);

#ifdef __cplusplus
}
#endif

#endif /* CNVMAP.1.17_CUDA_H */
