#ifndef HMB.1.0_CUDA_H
#define HMB.1.0_CUDA_H
...
/* Core hmb.1.0 CUDA Functions */
cudaError_t hmb.1.0_process_cuda(
    cuda_array_t *input_data,
    cuda_array_t *output_data,
    void *parameters
);

cudaError_t hmb.1.0_process_ranges_cuda(
...
/* Utility Functions */
bool hmb.1.0_cuda_is_available(void);
int hmb.1.0_cuda_get_device_count(void);
const char* hmb.1.0_cuda_get_error_string(cudaError_t error);
...
} hmb.1.0_cuda_profile_t;

cudaError_t hmb.1.0_cuda_enable_profiling(bool enable);
cudaError_t hmb.1.0_cuda_get_profile(hmb.1.0_cuda_profile_t *profile);

#ifdef __cplusplus
}
#endif

#endif /* HMB.1.0_CUDA_H */
